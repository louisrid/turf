...
export function resolveShoot(m, cell, dive) {
  ...
  const easyPen = (m.easyBotTeam === s.team) ? 0.20 : 0;
  const pen = 0.12 * rowOff + 0.18 * edge + easyPen;
  // SHO: BAD 72 / GOOD 82 / SUPERB 92. Open goal still needs finishing; SHO can beat a correct dive.
  const pOpen = clamp(0.55 + (s.sho - 72) * 0.018 - pen, 0.30, 0.97);
  const pCovered = clamp((s.sho - 72) * 0.012 - pen * 0.5, 0, 0.42);
...
export const longChance = (pas) => clamp(0.45 + (pas - 72) * 0.022, 0.25, 0.95);
