...
// ---- formation --------------------------------------------------------------
const SLOTS0 = [[2, 7], [1, 5], [3, 5]];  // normal: tight 5-wide pitch, GK central (bottom team)
const SLOTS1 = [[2, 0], [3, 2], [1, 2]];  // mirror (top team)
const FSLOTS0 = [[3, 7], [1, 6], [5, 6]]; // freddie 7-wide: GK central back + 2 deep spread
const FSLOTS1 = [[3, 0], [5, 1], [1, 1]]; // mirror

const MODE_CFG = {
  normal:  { mode: 'normal',  freddie: false, cols: 5, goalCols: [1, 2, 3] },
...
function resolveMode(mode) {
  ...
  return MODE_CFG[mode] || MODE_CFG.normal;         // unknown/legacy (e.g. 'louis2') -> normal
}
function slotsFor(team, cols) {
  const a = cols === 7 ? [FSLOTS0, FSLOTS1] : [SLOTS0, SLOTS1];
...
export function resolveShoot(m, cell, dive) {
  ...
  // SHO: BAD 72 / GOOD 82 / SUPERB 92. Rarity now swings scoring hard: ~30% (BAD) -> ~75% (SUPERB).
  const pOpen = clamp(0.55 + (s.sho - 72) * 0.021 - pen, 0.40, 0.98);
  const pCovered = clamp(0.05 + (s.sho - 72) * 0.024 - pen * 0.4, 0.02, 0.55);
