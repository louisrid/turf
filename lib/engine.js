...
export const PASS_SHORT = 3;   // reliable, can be cut on the lane
...
export function moveAllowance(m, p) {
  ...
  return m.ball.carrier === p.id ? 1 : 2;   // carry 1, off-ball 2
}
export const canShootFrom = (team, row) => row === goalRowFor(team);
...
export function createMatch(squadA, squadB) {
  ...
  const m = {
    ...
    possession: 0, score: [0, 0], turn: 1, phase: 'PLANNING', pending: null,
  };
  kickoff(m, 0);
  return m;
}
...
function kickoff(m, toTeam) {
  ...
  m.possession = toTeam; m.pending = null; m.phase = 'PLANNING';
}
...
export function sanitizeOrders(m, team, orders) {
  ...
  for (const p of m.players) {
    ...
    } else if (o.type === 'longpass' && isCarrier && attacking) {
...
function resolvePass(m, cr, target, dest, events, lofted) {
  events.push({ t: lofted ? 'longpass' : 'pass', from: { col: cr.col, row: cr.row }, to: pt(target) });
  cr.hasBall = false;
  if (!lofted) {
  ...
  // lofted, or short pass into empty space: lands loose
...
function relocate(m, p, from) {
  ...
  const cands = [
    { col: from.col, row: from.row + step }, { col: from.col + s, row: from.row + step }, { col: from.col - s, row: from.row + step },
    { col: from.col + s, row: from.row }, { col: from.col - s, row: from.row }, { col: from.col, row: from.row - step },
  ];
  for (const c of cands) { if (!inBounds(c.col, c.row)) continue; if (m.players.some(q => q.id !== p.id && q.col === c.col && q.row === c.row)) continue; p.col = c.col; p.row = c.row; return; }
  p.row = clamp(from.row + step, 0, ROWS - 1); if (p.col === from.col && p.row === from.row) p.col = clamp(from.col + s, 0, COLS - 1);
}
...
function deoverlap(m) {
  for (let pass = 0; pass < 3; pass++) {
    ...
    for (let i = 0; i < m.players.length; i++) for (let j = i + 1; j < m.players.length; j++) {
      ...
      if (a.col === b.col && a.row === b.row && a.team !== b.team) { const mv = a.hasBall ? b : (b.hasBall ? a : (Math.random() < 0.5 ? a : b)); relocate(m, mv, { col: a.col, row: a.row }); any = true; }
    }
    if (!any) break;
  }
}
...
export function resolveShoot(m, cell, dive) {
  ...
  let goal;
  if (!covered) goal = Math.random() < clamp(0.55 + (s.sho - 78) / 100, 0.30, 0.90); // open: SHO converts, low SHO can skew wide
  else goal = Math.random() < clamp((s.sho - 82) / 45, 0, 0.40);                       // power past a correct dive
