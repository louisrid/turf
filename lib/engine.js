...
export function resolveOrders(m, ordersA, ordersB) {
  ...
  if (cr && co && (co.type === 'pass' || co.type === 'longpass')) {
    ...
    if (m.pending && m.pending.kind === 'shoot') {   // long ball controlled in scoring range -> instant 1v1
      deoverlap(m); m.phase = 'SHOOT';
      return { events, pending: m.pending };
    }
    deoverlap(m);
...
function resolvePass(m, cr, target, dest, events, lofted) {
  ...
  if (!lofted) {
    ...
    if (rcv) {
      // a defender within 1 square of the receiver reads it, steps onto the receiving square and takes the ball
      ...
      if (cut) {
        ...
        if (dp.col !== rpos.col || dp.row !== rpos.row) events.push({ t: 'move', id: def.id, from: { col: def.col, row: def.row }, to: { col: rpos.col, row: rpos.row } });
        def.col = rpos.col; def.row = rpos.row;          // pulled onto the nearest intercepting square
        if (rcv.col === rpos.col && rcv.row === rpos.row) scatter(m, rcv, rpos, events);  // bump the denied receiver if they were standing there
        giveBall(m, def, events);
        events.push({ t: 'intercept', id: def.id, at: pt(rpos) });
        return new Set([def.id, rcv.id]);              // positions finalised; moveSimple leaves them
      }
  ...
  if (mate) {
    ...
    if (cut) {
      const def = cut.p, dp = dest.get(def.id);
      if (dp.col !== md.col || dp.row !== md.row) events.push({ t: 'move', id: def.id, from: { col: def.col, row: def.row }, to: { col: md.col, row: md.row } });
      def.col = md.col; def.row = md.row;
      if (mate.p.col === md.col && mate.p.row === md.row) scatter(m, mate.p, md, events);
      giveBall(m, def, events);
      events.push({ t: 'intercept', id: def.id, at: pt(md) });
      return new Set([def.id, mate.p.id]);
    }
    if (Math.random() < longChance(cr.pas)) { // PAS: higher = reliably finds the man, not loose
      giveBall(m, mate.p, events);
      if (canShootFrom(mate.p.team, md.row)) {          // landed in behind, in scoring range -> instant shootout
        const gk = gkOf(m, 1 - mate.p.team);
        m.pending = { kind: 'shoot', shooterId: mate.p.id, gkId: gk ? gk.id : null, at: pt(md) };
        events.push({ t: 'control', id: mate.p.id, at: pt(md), inBehind: true });
        events.push({ t: 'shootStart', shooterId: mate.p.id, gkId: gk ? gk.id : null, at: pt(md) });
      } else {
