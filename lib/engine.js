...
function slotsFor(team, freddie) { return freddie ? (team === 0 ? FSLOTS0 : FSLOTS1) : (team === 0 ? SLOTS0 : SLOTS1); }

function place(squad, team, freddie) {
  const slots = slotsFor(team, freddie);
...
export function createMatch(squadA, squadB, freddie = false) {
  const players = [...place(squadA, 0, freddie), ...place(squadB, 1, freddie)];
  const m = {
    players, ball: { carrier: null, loose: null },
    possession: 0, score: [0, 0], turn: 1, phase: 'PLANNING', pending: null, longCd: [0, 0],
    freddie: !!freddie, cols: freddie ? 7 : COLS, goalCols: freddie ? [2, 3, 4] : GOAL_COLS, keeper: [null, null],
  };
  kickoff(m, 0);
  return m;
}
...
function kickoff(m, toTeam) {
  ...
  for (const p of m.players) {
    ...
    const sl = slotsFor(p.team, m.freddie)[p.team === 0 ? i0++ : i1++];
...
export function snapshot(m) {
  return {
    cols: m.cols, rows: ROWS, goalCols: m.goalCols, freddie: !!m.freddie,
...
function resolvePass(m, cr, target, dest, events, lofted) {
  ...
  if (!lofted) {
    ...
    if (rcv) {
      const rpos = dest.get(rcv.id);
      // interception happens ONLY when the targeted receiver has an enemy within 1 of where the ball arrives
      const cut = m.players.filter(p => p.team !== cr.team)
        .map(p => ({ p, d: cheb(dest.get(p.id), rpos) }))
        .filter(x => x.d <= 1)
        .sort((a, b) => a.d - b.d)[0];
      if (cut) {
        const def = cut.p, dp = dest.get(def.id);
        giveBall(m, def, events);                        // defender wins the ball where THEY ran to (no teleport)
        events.push({ t: 'intercept', id: def.id, at: pt(dp) });
        if (dp.col === rpos.col && dp.row === rpos.row) {            // both ran to the same square
          const rO = { col: rcv.col, row: rcv.row };                // the defender takes it; the runner ends up one square off
          const spot = oneOffSquare(m, rpos, rO, dest, rcv.id);
          if (spot) { rcv.col = spot.col; rcv.row = spot.row; }
          return new Set([rcv.id]);
        }
        return;                                          // otherwise everyone moves normally; the receiver just gets no ball
      }
  ...
  if (mate) {
    ...
    // a defender within 1 square of the receiver reads the long ball, steps onto the drop square and takes it
    const cut = m.players.filter(p => p.team !== cr.team)
      .map(p => ({ p, d: cheb(dest.get(p.id), md) }))
      .filter(x => x.d <= 1)
      .sort((a, b) => a.d - b.d)[0];
    if (cut) {
      const def = cut.p, dp = dest.get(def.id);
      giveBall(m, def, events);                          // defender wins it where they ran to (no teleport)
      events.push({ t: 'intercept', id: def.id, at: pt(dp) });
      if (dp.col === md.col && dp.row === md.row) {                  // same square
        const rO = { col: mate.p.col, row: mate.p.row };
        const spot = oneOffSquare(m, md, rO, dest, mate.p.id);
        if (spot) { mate.p.col = spot.col; mate.p.row = spot.row; }
        return new Set([mate.p.id]);
      }
      return;
    }
...
function oneOffSquare(m, center, origin, dest, skipId) {
  const finalOcc = (c, r) => m.players.some(q => q.id !== skipId && (() => { const x = dest.get(q.id) || { col: q.col, row: q.row }; return x.col === c && x.row === r; })());
