...
import { ROWS, PASS_SHORT, PASS_LONG, shortPassOk, moveAllowance, canShootFrom, keeperOut } from './engine.js';
...
function carrierOrderOk(m, cr, o) {
  ...
  if (o.type === 'shoot') return canShootFrom(cr.team, cr.row) || (m.freddie && keeperOut(m, 1 - cr.team));
  if (o.type === 'pass') return !!o.to && shortPassOk(cr, o.to) && !same(o.to);
...
function finalizeOrders(m, team, orders) {
  ...
  for (const p of mine) {
    ...
    for (let dc = -a; dc <= a; dc++) for (let dr = -a; dr <= a; dr++) {
      const c = cl(p.col + dc, 0, W - 1), r = cl(p.row + dr, 0, ROWS - 1);
...
export function botOrders(m, team, diff) {
  ...
  const orders = {};
  const W = m.cols, CEN = (m.cols - 1) / 2, GMIN = m.goalCols[0], GMAX = m.goalCols[m.goalCols.length - 1];
  const oppOut = () => keeperOut(m, 1 - team);            // freddie: opponent has no keeper back
  const inb = (c, r) => c >= 0 && c < W && r >= 0 && r < ROWS;
  const step = (p, to, range) => { const dc = Math.sign(to.col - p.col), dr = Math.sign(to.row - p.row); const n = Math.min(range, Math.max(Math.abs(to.col - p.col), Math.abs(to.row - p.row))); return { col: cl(p.col + dc * n, 0, W - 1), row: cl(p.row + dr * n, 0, ROWS - 1) }; };
  ...
  const positionKeeper = () => { if (gk && !orders[gk.id]) { const want = { col: cl(cr ? cr.col : Math.round(CEN), GMIN, GMAX), row: home }; if (!occ(want.col, want.row, gk.id) && cheb(gk, want) > 0) orders[gk.id] = { type: 'move', to: step(gk, want, 1) }; } };
  ...
  if (m.ball.loose) {
    ...
    if (near[1]) { const supp = { col: cl(lb.col, 0, W - 1), row: cl(lb.row + f, 0, ROWS - 1) }; if (!occ(supp.col, supp.row, near[1].id)) orders[near[1].id] = { type: 'move', to: step(near[1], supp, moveAllowance(m, near[1])) }; }
    if (gk) { if (cheb(gk, lb) <= 1 && Math.abs(lb.row - home) <= 1 && lb.col >= 1 && lb.col <= W - 2) orders[gk.id] = { type: 'move', to: step(gk, lb, 1) }; else positionKeeper(); }
  ...
  if (attacking && cr) {
    ...
    const shotQuality = (p) => 1 - 0.30 * Math.abs(p.row - gr) - (p.col === 0 || p.col === W - 1 ? 0.45 : (p.col === 1 || p.col === W - 2 ? 0.12 : 0));
    const canSh = (m.freddie || cr.pos !== 'GK') && (canShootFrom(team, cr.row) || (m.freddie && oppOut()));
    ...
    const forwardSquare = () => {
      ...
      for (let dc = -a; dc <= a; dc++) for (let dr = -a; dr <= a; dr++) {
        const c = cl(cr.col + dc, 0, W - 1), r = cl(cr.row + dr, 0, ROWS - 1);
        ...
        if (adv < 0) continue;
        const sc = adv * 2 + (2 - Math.abs(c - CEN)) * 0.8 + defDist(c, r) * 0.7;
    ...
    const betterShot = () => {
      ...
      for (let dc = -a; dc <= a; dc++) for (let dr = -a; dr <= a; dr++) {
        ...
        const q = 1 - 0.30 * Math.abs(r - gr) - 0.45 * ((c === 0 || c === W - 1) ? 1 : 0);
    ...
    const shortOutlet = () => {
      ...
      for (let dc = -ra; dc <= ra; dc++) for (let dr = -ra; dr <= ra; dr++) {
        const c = cl(runner.col + dc, 0, W - 1), r = cl(runner.row + dr, 0, ROWS - 1);
        if (cheb(runner, { col: c, row: r }) > ra) continue;
        if (occ(c, r, runner.id) || (c === cr.col && r === cr.row)) continue;
        if (!shortPassOk(cr, { col: c, row: r }) || !laneClear(cr, { col: c, row: r })) continue;
        const adv = (gr === 0 ? cr.row - r : r - cr.row);
        const sc = defDist(c, r) * 1.2 + adv * 0.8 + (2 - Math.abs(c - CEN)) * 0.4;
        if (sc > bs) { bs = sc; best = { col: c, row: r }; }
      }
      return best;
    };
    ...
    const longRun = () => {
      ...
      for (let dc = -ra; dc <= ra; dc++) for (let dr = -ra; dr <= ra; dr++) {
        ...
        const sc = adv * 1.5 + defDist(c, r) * 1.0 + (2 - Math.abs(c - CEN)) * 0.3;
        if (sc > bs) { bs = sc; best = { col: c, row: r }; }
      }
      return best;
    };

    if (m.freddie && oppOut() && canSh) {
      orders[cr.id] = { type: 'shoot' };               // open goal: take it from anywhere
    } else if (!hard && cr.pos !== 'GK' && Math.random() < 0.12 && !(canSh && shotQuality(cr) > 0.75)) {
      ...
      else if (d && shortPassOk(cr, d)) orders[cr.id] = { type: 'pass', to: { col: d.col, row: d.row } };
      else { const fs = forwardSquare(); if (fs) orders[cr.id] = { type: 'move', to: fs }; }
    } else if (!m.freddie && cr.pos === 'GK') {
      ...
      runnerTo = { col: cl(GMIN + Math.round(Math.random() * (GMAX - GMIN)), GMIN, GMAX), row: gr }; // crash for a rebound
    } else if (doubled) {
      const o = shortOutlet();
      if (o) { orders[cr.id] = { type: 'pass', to: o }; runnerTo = o; }
      else if (gk && shortPassOk(cr, gk) && laneClear(cr, gk)) orders[cr.id] = { type: 'pass', to: { col: gk.col, row: gk.row } };
      else { const lr = longOk ? longRun() : null; if (lr) { orders[cr.id] = { type: 'longpass', to: step(cr, lr, PASS_LONG) }; runnerTo = lr; } }
    } else if (pressed) {
      const o = shortOutlet();
      if (o) { orders[cr.id] = { type: 'pass', to: o }; runnerTo = o; }
      else { const fs = forwardSquare(); if (fs && defDist(fs.col, fs.row) >= 1) orders[cr.id] = { type: 'move', to: fs }; else if (gk && shortPassOk(cr, gk) && laneClear(cr, gk)) orders[cr.id] = { type: 'pass', to: { col: gk.col, row: gk.row } }; }
    } else {
    ...
    if (!carrierOrderOk(m, cr, orders[cr.id])) {
      ...
      const side = { col: cl(cr.col + (cr.col < CEN ? 1 : -1), 0, W - 1), row: cr.row };
      if (!foeOn(fwd.col, fwd.row) && !occ(fwd.col, fwd.row, cr.id) && !(fwd.col === cr.col && fwd.row === cr.row)) orders[cr.id] = { type: 'move', to: fwd };
      else if (cr.pos !== 'GK' && gk && shortPassOk(cr, gk)) orders[cr.id] = { type: 'pass', to: { col: gk.col, row: gk.row } };
    ...
    if (runner && !orders[runner.id]) {
      ...
      if (!to) {
        ...
        if (!to) to = longRun();
        if (!to) { const ra = moveAllowance(m, runner); to = step(runner, { col: cl(runner.col + (runner.col < CEN ? 1 : -1), 0, W - 1), row: cl(runner.row + f, 0, ROWS - 1) }, ra); }
      }
  ...
  if (!attacking && cr) {
    ...
    if (marker) {
      if (oppRunner) {
        ...
        const gs = { col: cl(GMIN + Math.round(Math.random() * (GMAX - GMIN)), GMIN, GMAX), row: cl(cr.row + df, 0, ROWS - 1) };
        orders[marker.id] = { type: 'move', to: step(marker, gs, moveAllowance(m, marker)) };
      }
    }
    if (gk) { const want = { col: cl(cr.col, GMIN, GMAX), row: home }; if (!occ(want.col, want.row, gk.id) && cheb(gk, want) > 0) orders[gk.id] = { type: 'move', to: step(gk, want, 1) }; }
    return finalizeOrders(m, team, orders);
  }
    return finalizeOrders(m, team, orders);
}
