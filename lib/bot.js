...
function carrierOrderOk(m, cr, o) {
  ...
  if (o.type === 'backpass') return cr.pos !== 'GK';
...
export function botOrders(m, team, diff) {
  ...
  if (attacking && cr) {
    ...
    const shotQuality = (p) => 1 - 0.30 * Math.abs(p.row - gr) - 0.45 * ((p.col === 0 || p.col === COLS - 1) ? 1 : 0) - 0.12 * Math.min(2, Math.abs(p.col - 2.5) - 0.5 < 0 ? 0 : Math.abs(p.col - 2.5) - 0.5);
    ...
    if (!hard && cr.pos !== 'GK' && Math.random() < 0.30 && !(canSh && shotQuality(cr) > 0.75)) {
      // EASY blunder: give the ball away into a defended area
      const d = defs.slice().sort((a, b) => cheb(a, cr) - cheb(b, cr))[0];
      if (d) orders[cr.id] = { type: 'longpass', to: step(cr, { col: d.col, row: d.row }, PASS_LONG) };
    } else if (cr.pos === 'GK') {
      const o = shortOutlet(); if (o) { orders[cr.id] = { type: 'pass', to: o }; runnerTo = o; }
      else if (runner) { const lr = longRun(); const tgt = lr || { col: runner.col, row: cl(runner.row + f, 0, ROWS - 1) }; orders[cr.id] = { type: 'longpass', to: step(cr, tgt, PASS_LONG) }; runnerTo = lr; }
      ...
      const lr = longRun(); if (lr) { orders[cr.id] = { type: 'longpass', to: step(cr, lr, PASS_LONG) }; runnerTo = lr; }
      else if (gk) orders[cr.id] = { type: 'backpass' };
      else { const o = shortOutlet(); if (o) { orders[cr.id] = { type: 'pass', to: o }; runnerTo = o; } }
    } else if (pressed) {
      const o = shortOutlet();
      if (o) { orders[cr.id] = { type: 'pass', to: o }; runnerTo = o; }
      else { const fs = forwardSquare(); if (fs && defDist(fs.col, fs.row) >= 1) orders[cr.id] = { type: 'move', to: fs }; else if (gk) orders[cr.id] = { type: 'backpass' }; }
    } else {
      const bs = canSh ? betterShot() : null;
      if (canSh && shotQuality(cr) < 0.55 && bs) orders[cr.id] = { type: 'move', to: bs };
      else if (hard && longRun() && Math.random() < 0.25) { const lr = longRun(); orders[cr.id] = { type: 'longpass', to: step(cr, lr, PASS_LONG) }; runnerTo = lr; }
    ...
    if (!carrierOrderOk(m, cr, orders[cr.id])) {
      ...
      else if (cr.pos !== 'GK' && gk) orders[cr.id] = { type: 'backpass' };
  ...
  if (!attacking && cr) {
    ...
    const neigh = (p) => { const o = []; for (let dc = -1; dc <= 1; dc++) for (let dr = -1; dr <= 1; dr++) { if (!dc && !dr) continue; const c = p.col + dc, r = p.row + dr; if (inb(c, r)) o.push({ col: c, row: r }); } return o; };
    ...
    const pressReach = hard ? 3 : 1;

    if (presser && presser.recover === 0) {
      const commit = cheb(presser, cr) <= pressReach && (hard ? (cornered || cheb(presser, cr) <= 2) : cheb(presser, cr) <= 1);
      if (commit) orders[presser.id] = { type: 'winball' };
      else { const gs = { col: cr.col, row: cl(cr.row + df, 0, ROWS - 1) }; orders[presser.id] = { type: 'move', to: step(presser, gs, moveAllowance(m, presser)) }; }
    }
    if (marker && marker.recover === 0) {
      ...
      if (hard && cornered && !runnerThreat && cheb(marker, cr) <= 3) orders[marker.id] = { type: 'winball' };
      else if (oppRunner) {
        // sit goal-side of the runner; on hard, tuck slightly toward the passing lane
        const gsRow = cl(oppRunner.row + df, 0, ROWS - 1);
        let tgt = { col: oppRunner.col, row: gsRow };
        if (!hard) tgt = { col: oppRunner.col, row: cl(oppRunner.row + df * 2, 0, ROWS - 1) }; // easy: drop off, give space
        orders[marker.id] = { type: 'move', to: step(marker, tgt, moveAllowance(m, marker)) };
      } else { const gs = { col: cl(2 + Math.round(Math.random()), 2, 3), row: cl(cr.row + df, 0, ROWS - 1) }; orders[marker.id] = { type: 'move', to: step(marker, gs, moveAllowance(m, marker)) }; }
    }
