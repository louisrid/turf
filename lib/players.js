...
function lookFor(name) {
  const h = hash(name);
  return { skin: SKINS[h % SKINS.length], hair: HAIRSTYLES[(Math.floor(h / 8)) % HAIRSTYLES.length], hairColor: HAIRCOLORS[(Math.floor(h / 64)) % HAIRCOLORS.length] };
}
...
export function rarity(ovr) { return ovr >= 92 ? 'icon' : ovr >= 87 ? 'gold' : ovr >= 82 ? 'silver' : 'bronze'; }
