...
const DIST = {
  easyPack: [['BAD', 0.06], ['GOOD', 0.49], ['SUPERB', 0.45]],   // beat easy AI: very good
  pvpPack:  [['BAD', 0.03], ['GOOD', 0.42], ['SUPERB', 0.55]],   // beat a human: very good
  hardPack: [['BAD', 0.01], ['GOOD', 0.29], ['SUPERB', 0.70]],   // beat hard AI: very very good
...
export function makeStartingSquad() {
  return [makePlayer('GK', TIER.BAD, TIER.BAD), makePlayer('FWD', TIER.BAD, TIER.BAD), makePlayer('FWD', TIER.BAD, TIER.BAD)];
}

// Bot squad scaled by difficulty.
export function makeSquadFor(diff) {
  const d = diff === 'hard' ? DIST.hardBot : DIST.easyBot;
  return [makeFromDist('GK', d), makeFromDist('FWD', d), makeFromDist('FWD', d)];
}
export const makeSquadNear = () => makeStartingSquad();   // legacy fallback

// Win reward: ONE player, tiers weighted by difficulty.
const PACK_TIER = { easy: 'easyPack', pvp: 'pvpPack', hard: 'hardPack' };
export function makePack(diff) {
  const d = DIST[PACK_TIER[diff] || 'easyPack'];
  const pos = Math.random() < 0.15 ? 'GK' : 'FWD';
  let sho = drawTier(d), pas = drawTier(d);
  if (sho === TIER.BAD && pas === TIER.BAD) { if (Math.random() < 0.5) sho = TIER.GOOD; else pas = TIER.GOOD; }  // never a BAD/BAD reward
  return [makePlayer(pos, sho, pas)];
}
